...
boolean isMember(Dictionary D, int x)
{
	...
	if (D[hashval] != x)
	{
		...
		// stops when temp wraps back to hashval
		for (temp = hashval + 1; temp != hashval && D[temp] != x; temp = (temp + 1) % SIZE)
		{
		}
	}

	return retval = (temp != hashval) ? TRUE : FALSE;
}
...
void insertElem(Dictionary D, int x)
{
	...
	for (count = 0; D[hashval] != EMPTY && D[hashval] != DELETED && D[hashval] != x;
		 hashval = (hashval + 1) % SIZE, count++)
	{
	}

	if (count < SIZE)
	{
		if (D[hashval] != x) // avoid duplicate no matter where it is
			D[hashval] = x;
	}
}
