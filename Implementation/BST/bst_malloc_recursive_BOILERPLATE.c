...
void INSERT_BST(BST *T, int elem) {
    ...
    for (trav = T; *trav != NULL && (*trav)->data != elem; ){
         if (elem > (*trav)->data) trav = &(*trav)->RC;
        else  trav = &(*trav)->LC;
    }
...
int MAX_BST(BST T) {
     if (T == NULL) return -1;
    BST trav = T;
    while (trav->RC != NULL) trav = trav->RC;
    return trav != NULL ? trav->data : -1;
...
void DELETE_BST(BST *T, int elem) {
    ...
    if (*trav != NULL) {  // Found the node
        BST temp = *trav;
        if (temp->LC == NULL) {  // No left child or leaf
            *trav = temp->RC;
        } else if (temp->RC == NULL) {  // No right child
            *trav = temp->LC;
...
boolean MEMBER_BST(BST T, int elem) {
    ...
    for (BST trav = T; trav != NULL; trav = (elem < trav->data) ? trav->LC : trav->RC) if (elem == trav->data) return TRUE;
...
void LEVEL_ORDER_BST(BST T) {
    ...
    while (front < rear)
    {
        ...
        if (curr->LC != NULL) queue[rear++] =curr->LC;
        if (curr->RC != NULL) queue[rear++] =  curr->RC;
